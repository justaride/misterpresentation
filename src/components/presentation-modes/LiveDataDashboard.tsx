...
const DEFAULT_SSE_URL = import.meta.env.VITE_LIVE_SSE_URL || "";
const DEFAULT_WS_URL = import.meta.env.VITE_LIVE_WS_URL || "";

function defaultUrlForSource(source: DataSource) {
  if (source === "sse") return DEFAULT_SSE_URL;
  if (source === "ws") return DEFAULT_WS_URL;
  return "";
}
...
function toTs(value: unknown, fallback: number): number {
  const n = toNum(value);
...
function parseSource(value: string | null): DataSource {
  if (value === "sse" || value === "ws") return value;
  return "sim";
}
...
function toSeverity(value: unknown): FeedSeverity | undefined {
...
function toEventType(value: unknown): FeedEventType | undefined {
  const s = (toStr(value) ?? "").toLowerCase();
  if (s.includes("signup")) return "signup";
  if (s.includes("purchase") || s.includes("paid") || s.includes("sale"))
    return "purchase";
  if (s.includes("deploy") || s.includes("release")) return "deploy";
  if (s.includes("incident") || s.includes("alert") || s.includes("error"))
    return "incident";
  return undefined;
}

function coerceChannels(
  value: unknown,
  fallback?: Record<Channel, number>,
  ...
  const out = { ...base } as Record<Channel, number>;
  for (const ch of CHANNELS) {
    const n = toNum(value[ch]);
    if (typeof n === "number") out[ch] = n;
  }
...
function coercePoint(
  ...
  const safeSignups = Math.max(0, Math.round(signups ?? 0));
  const safeConversions = Math.max(
    0,
    Math.round(Math.min(conversions ?? 0, safeSignups)),
  );

  return {
    ts: toTs(value.ts ?? value.timestamp, now),
    activeUsers: Math.max(0, Math.round(activeUsers ?? 0)),
    signups: safeSignups,
    conversions: safeConversions,
    revenue: Math.max(0, Math.round(revenue ?? 0)),
    latencyP95: Math.max(0, Math.round(latencyP95 ?? 0)),
    errorRate,
    channels,
  };
}
...
function coerceEvents(
  ...
  const coerceOne = (v: unknown): FeedEvent | null => {
    if (!isRecord(v)) return null;
    const message = toStr(v.message ?? v.text ?? v.title);
    if (!message) return null;

    const severity = toSeverity(v.severity ?? v.level) ?? "info";
    const type = toEventType(v.type ?? v.eventType ?? v.kind) ?? "incident";
    const id = toStr(v.id) ?? `evt_ext_${(idRef.current += 1)}`;

    return {
      id,
      ts: toTs(v.ts ?? v.timestamp, now),
      severity,
      type,
      message,
    };
  ...
  if (Array.isArray(value)) {
    return value.map(coerceOne).filter((v): v is FeedEvent => !!v);
  }
  const one = coerceOne(value);
  return one ? [one] : [];
}
...
function connectionBadgeClasses(state: ConnectionState) {
  if (state === "connected")
    return "bg-emerald-300/15 text-emerald-100 border-emerald-300/30";
  if (state === "connecting")
    return "bg-cyan-300/15 text-cyan-100 border-cyan-300/30";
  if (state === "error")
    return "bg-rose-500/15 text-rose-200 border-rose-500/30";
  if (state === "simulated")
    return "bg-violet-300/15 text-violet-100 border-violet-300/30";
  return "bg-white/5 text-slate-200/70 border-white/10";
}
...
export function LiveDataDashboard() {
  ...
  const initialUrl =
    ...
    (initialSource === "sim" ? "" : defaultUrlForSource(initialSource));

  const [source, setSource] = useState<DataSource>(initialSource);
  const [url, setUrl] = useState(initialUrl);
  const [connection, setConnection] = useState<ConnectionStatus>(() => ({
    state: initialSource === "sim" ? "simulated" : "disconnected",
  ...
  const reset = useCallback(() => {
    ...
    setFeed([]);
    setConnection((c) => ({ ...c, lastMessageAt: undefined, lastError: undefined }));
  }, []);
  ...
  useEffect(() => {
    const nextSource = parseSource(searchParams.get("source"));
    const nextUrl =
      (searchParams.get("url") ?? "") ||
      (nextSource === "sim" ? "" : defaultUrlForSource(nextSource));
    setSource((prev) => (prev === nextSource ? prev : nextSource));
    setUrl((prev) => (prev === nextUrl ? prev : nextUrl));
  }, [searchParams]);

  useEffect(() => {
    const currentSource = parseSource(searchParams.get("source"));
    const currentUrl = searchParams.get("url") ?? "";
    if (currentSource === source && currentUrl === url) return;
  ...
  }, [searchParams, setSearchParams, source, url]);

  useEffect(() => {
    if (source === "sim") setConnection({ state: "simulated" });
  }, [source]);
  ...
  const ingest = useCallback(
    (payload: unknown) => {
      ...
      let pointValue: unknown = payload;
      let eventsValue: unknown = null;

      if (isRecord(payload)) {
        const t = (toStr(payload.type) ?? "").toLowerCase();
        if (payload.point != null) pointValue = payload.point;
        else if (t === "point" && payload.data != null) pointValue = payload.data;
        else if (t === "event" || t === "events") pointValue = null;

        eventsValue =
          payload.events ??
          payload.event ??
          (t === "event" || t === "events" ? payload.data ?? payload : null);
      }

      const externalEvents =
        eventsValue != null ? coerceEvents(eventsValue, now, eventIdRef) : [];

      if (externalEvents.length > 0 && runningRef.current) {
        setFeed((prevFeed) => [...externalEvents, ...prevFeed].slice(0, 22));
      }

      if (pointValue != null) {
        setPoints((prevPoints) => {
          const fallback = prevPoints[prevPoints.length - 1];
          const nextPoint = coercePoint(pointValue, fallback, now);
          if (!nextPoint) return prevPoints;

          if (runningRef.current && externalEvents.length === 0) {
            setFeed((prevFeed) => {
              const added = generateEvents({
                now,
                point: nextPoint,
              ...
              if (added.length === 0) return prevFeed;
              return [...added, ...prevFeed].slice(0, 22);
            });
          }
      ...
      if (externalEvents.length > 0 || pointValue != null) {
        setConnection((c) => ({ ...c, lastMessageAt: now, lastError: undefined }));
      }
    },
    [setFeed, setPoints],
  );
  ...
  useEffect(() => {
    ...
    if (!url) {
      setConnection({ state: "disconnected", lastError: "Missing SSE URL" });
      return;
    }

    const resolved = resolveHttpUrl(url);
    setConnection({ state: "connecting", url: resolved });

    const es = new EventSource(resolved);
    es.onopen = () => {
      setConnection((c) => ({ ...c, state: "connected", url: resolved, lastError: undefined }));
    };
    es.onmessage = (evt) => {
      ...
      if (parsed == null) {
        ingest({ type: "event", message: text, severity: "info" });
        return;
      }
      ingest(parsed);
    };
    es.onerror = () => {
      setConnection((c) => ({ ...c, state: "error", url: resolved, lastError: "SSE error" }));
    };

    return () => {
      es.close();
      setConnection((c) => ({ ...c, state: "disconnected", url: resolved }));
    };
  }, [ingest, source, url]);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (evt) => {
      ...
      if (parsed == null) {
        ingest({ type: "event", message: evt.data, severity: "info" });
        return;
      }
      ingest(parsed);
    };
    ws.onerror = () => {
      setConnection((c) => ({ ...c, state: "error", url: resolved, lastError: "WebSocket error" }));
    };
    ws.onclose = (evt) => {
      setConnection((c) => ({
        ...c,
        ...
        lastError:
          evt.code === 1000
            ? undefined
            : evt.reason || `WebSocket closed (${evt.code})`,
      }));
    };
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[11px] font-mono text-slate-400/90">
                  ...
                  {source === "sim" ? `${speed}x` : source.toUpperCase()} •{" "}
                  {source === "sim" ? "simulated" : connection.state}
                </span>
              </div>
              <p className="text-sm text-slate-300/70 max-w-2xl">
                Operational KPI snapshot with simulated or streamed real-time
                updates (SSE/WebSocket). Use{" "}
                    ...
                    {[1, 2, 5].map((s) => (
                      <button
                        ...
                        className={clsx(
                          ...
                          source !== "sim"
                            ? "opacity-30 cursor-not-allowed bg-white/5 border-white/10 text-slate-200/60"
                            : speed === s
                ...
                <div className="px-3 py-2 flex flex-col gap-2">
                  <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            setUrl((prevUrl) => {
                              ...
                              const prevDefault = defaultUrlForSource(prevSource);
                              if (!trimmed || trimmed === prevDefault) {
                                return defaultUrlForSource(s);
                              }
                              return trimmed;
                            });
                          }}
                          className={clsx(
                            "px-2.5 py-2 rounded-lg border text-[12px] font-mono transition-colors uppercase",
                            source === s
                              ? "bg-cyan-300/15 border-cyan-300/30 text-cyan-100"
                              : "bg-white/5 border-white/10 text-slate-200/80 hover:bg-white/10",
                          )}
                    ...
                    <span
                      ...
                      className={clsx(
                        "inline-flex items-center px-2 py-1 rounded-full border text-[11px] font-mono uppercase tracking-wider",
                        connectionBadgeClasses(connection.state),
                      )}
                  ...
                  {source !== "sim" && (
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        placeholder={
                          source === "sse" ? "/api/live" : "/api/live/ws"
                        }
                        className="w-80 max-w-[60vw] px-3 py-2 rounded-xl bg-slate-950/40 border border-white/10 text-[12px] font-mono text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-300/30 focus:border-cyan-300/30"
                        spellCheck={false}
                        inputMode="url"
                      />
                    </div>
