...
import { ChevronLeft, ChevronRight, Play, CheckCircle2, Copy, Code } from "lucide-react";
...
const STEPS: CodeStep[] = [
  ...
  },
  {
    ...
    subtitle: "Move the useState and useEffect into a new function starting with 'use'.",
    type: "single",
    highlightLines: [1, 11],
...
export function LiveCodeWalkthrough() {
  ...
  const next = useCallback(() => setCurrent((s) => Math.min(s + 1, STEPS.length - 1)), []);
        ...
        <main className="flex-1 grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-8 overflow-hidden">
          ...
          <div className="flex flex-col h-full overflow-hidden">
            <div className="flex items-center gap-2 mb-2 px-4 py-2 bg-[#161B22] border border-slate-800 rounded-t-xl text-xs font-mono">
              ...
              <span>{step.id === "extract" ? "useWindowWidth.js" : "WindowWidth.jsx"}</span>
            </div>
            ...
            <div className="bg-[#161B22] border border-slate-800 rounded-2xl p-8 flex flex-col items-center justify-center text-center shadow-2xl">
              ...
              <div className="p-8 bg-black/40 rounded-3xl border border-slate-800 w-full">
                 <div className="text-5xl font-mono text-white mb-2">
                   {windowWidth}<span className="text-cyan-400 text-xl ml-1">px</span>
                 </div>
                 <div className="text-slate-500 text-sm uppercase tracking-tighter">Current Width</div>
              </div>
            </div>
            ...
            <div className="flex-1 bg-gradient-to-br from-indigo-900/20 to-cyan-900/20 border border-slate-800 rounded-2xl p-8 relative overflow-hidden group">
               <div className="relative z-10 flex flex-col h-full">
                  ...
                  <div className="space-y-4 text-slate-400">
                     {current === 0 && (
                        <p>Notice the repetitive logic required to manage the resize event listener. This component is doing too many things.</p>
                     )}
                     {current === 1 && (
                        <p>If we add this to every responsive component, our codebase will quickly become bloated and hard to maintain.</p>
                     )}
                     {current === 2 && (
                        <p>Hooks allow us to extract component logic into reusable functions. This function is now independent of any UI.</p>
                     )}
                     {current === 3 && (
                        <p>Look how much simpler the component is now. It describes *what* to show, not *how* to calculate the data.</p>
                     )}
                     {current === 4 && (
                        <div className="flex flex-col items-center gap-6 mt-8">
                           <CheckCircle2 size={64} className="text-green-400" />
                           <p className="text-center font-bold text-white">Refactoring Complete!</p>
                        </div>
                     )}
                  </div>
               </div>
               {/* Decorative Background Icon */}
               <Copy size={200} className="absolute -bottom-10 -right-10 text-white/5 opacity-10 rotate-12 group-hover:rotate-0 transition-transform duration-700" />
            </div>
          </aside>
        </main>
          ...
          <div className="flex items-center gap-4">
             <button
               ...
               className={`p-3 rounded-xl border border-slate-800 transition-all ${
                 current === 0 ? "opacity-20 cursor-not-allowed" : "hover:bg-slate-800 hover:text-white"
               }`}
             >
               <ChevronLeft size={24} />
             </button>
             <button
               ...
               className={`px-8 py-3 rounded-xl bg-cyan-600 text-white font-bold transition-all flex items-center gap-2 ${
                 current === STEPS.length - 1 ? "opacity-20 cursor-not-allowed" : "hover:bg-cyan-500 hover:scale-105 active:scale-95"
               }`}
             >
