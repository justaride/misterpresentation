...
function hash11(x: number) {
  // Cheap hash: deterministic but not cryptographic.
  const s = Math.sin(x * 127.1) * 43758.5453123;
  return s - Math.floor(s);
}

function generateTextPoints(text: string, countHint: number) {
  // Produces a pool of XY points in [-1, 1] space sampled from canvas pixels.
  const w = 900;
  const h = 260;
  const canvas = document.createElement("canvas");
  canvas.width = w;
  ...
  // Sample every ~2-3 pixels for speed; adjust based on countHint.
  const step = Math.max(2, Math.floor(Math.sqrt((w * h) / Math.max(countHint, 1)) * 0.75));
  for (let y = 0; y < h; y += step) {
    for (let x = 0; x < w; x += step) {
      const i = (y * w + x) * 4;
      const v = img[i]; // R channel; white text => 255
      if (v > 40) {
        const nx = (x / w) * 2 - 1;
        const ny = (1 - y / h) * 2 - 1;
        pts.push([nx, ny]);
...
const FRAG = /* glsl */ `
  ...
  void main() {
    ...
    float sparkle = 0.7 + 0.3 * sin(uTime * 6.0 + vSeed * 17.0);
...
type ParticleState = {
  ...
  // For boids: spatial hash scratch (rebuilt every frame in that mode)
...
function createParticleState(count: number): ParticleState {
  ...
  // Attractor state seeded near origin
  ...
  // Spatial hash (grid) for boids: fixed bucket count.
  // 32^3 grid => 32768 cells.
...
function usePostFX({
  ...
  useEffect(() => {
    ...
    const trailsEnabled = fx.trailsDamp < 0.999 && !reducedMotion && !degraded;
    after.enabled = trailsEnabled;
    after.uniforms.damp.value = trailsEnabled ? fx.trailsDamp : 1.0;

    bloom.enabled = !degraded;
    bloom.strength = degraded ? 0.35 : fx.bloomStrength;
    bloom.radius = fx.bloomRadius;
    bloom.threshold = fx.bloomThreshold;
  }, [fx, degraded, reducedMotion]);
...
function ParticleSystem({
  ...
  const [meaningPoints, webglPoints] = useMemo(() => {
    // Precompute point pools. Done once per mount.
    // Use count as hint so we sample a reasonable density.
    const pts1 = generateTextPoints("MEANING", count);
    const pts2 = generateTextPoints("WEBGL", count);
    return [pts1, pts2];
  }, [count]);

  // Per-slide initialization
  useEffect(() => {
    const st = stateRef.current;
    const { targets, velocities, attractX, attractY, attractZ, count: n } = st;

    // Reset velocities on slide enter (except where we want continuity).
    for (let i = 0; i < n * 3; i++) velocities[i] *= 0.25;

    // Reset attractor state when entering attractor slide.
    if (SLIDES[slideIndex]?.id === "strange-attractor") {
      for (let i = 0; i < n; i++) {
    ...
    // Reset targets depending on formation.
    ...
    for (let i = 0; i < n; i++) {
      ...
      switch (id) {
        ...
        case "curl-flow": {
          // Start from a stretched helix column.
        ...
        case "burst": {
          ...
          // Give a slide-enter impulse later in frame loop.
          break;
        }
        case "flock": {
  ...
  }, [slideIndex, meaningPoints, webglPoints]);

  // Restart signal: re-seed positions/velocities and re-init targets for current slide.
  useEffect(() => {
    const st = stateRef.current;
  ...
  useFrame((_state, dt) => {
    ...
    // Default spring-to-target parameters per mode.
    let k = 8.0;
    let c = 2.8;
    let maxSpeed = 6.5;

    if (id === "burst" || id === "singularity") {
      k = 11.0;
      c = 2.6;
      maxSpeed = 9.0;
    }
    if (id === "curl-flow" || id === "vortex-column" || id === "strange-attractor") {
      k = 6.5;
      c = 2.4;
      maxSpeed = 8.0;
    }
    if (id === "meaning" || id === "webgl") {
      k = 12.0;
      c = 3.6;
      maxSpeed = 6.0;
    }

    // Special: burst impulse on enter (first ~0.4s)
    const burstImpulse = id === "burst" ? smoothstep(0.0, 0.35, (performance.now() % 10_000) / 1000) : 0;

    // Gravity wells positions (static)
    const wellA = tmpVec.set(-2.2, 1.6, 0.2);
    const wellB = tmpVec2.set(2.4, -1.2, -0.4);
    const wellC = tmpTarget.set(0.4, 0.6, 2.1);

    // For boids: build spatial hash when in flock mode.
    if (id === "flock") {
      st.cellHeads.fill(-1);
      const gridSize = 32;
      const cellSize = 0.8;
    ...
    for (let i = 0; i < n; i++) {
      ...
      // Base target (static array) + some modes add dynamic deformation.
      let tx = targets[i3];
      let ty = targets[i3 + 1];
      let tz = targets[i3 + 2];

      if (id === "wave-surface") {
        const phase = px * 0.8 + py * 0.9 + t * 1.2;
        tz = Math.sin(phase) * 0.55 + Math.sin(phase * 0.45) * 0.25;
      }
      ...
      // Extra forces by mode
      if (id === "curl-flow") {
        // Cheap divergence-ish swirl field based on trig (acts like a flow field).
      ...
      if (id === "gravity-wells") {
        // Pull to nearest well with soft repulsion at close distance.
        const strength = 2.4;
        const repel = 0.5;
        // Well A
        let wx = wellA.x - px;
        ...
        // Local repulsion to avoid collapse
      ...
      if (id === "repulsion-lattice") {
        // Approx: repel from a moving anchor derived from seed to create spacing tension.
        const s = st.seeds[i];
      ...
      if (id === "flock") {
        // Cheap boids via spatial hash neighborhood (cell + adjacent cells).
        const gridSize = 32;
        const cellSize = 0.8;
        const gx = Math.max(0, Math.min(gridSize - 1, Math.floor((px + 6.5) / cellSize)));
        const gy = Math.max(0, Math.min(gridSize - 1, Math.floor((py + 6.5) / cellSize)));
      ...
      if (id === "strange-attractor") {
        // Lorenz attractor per particle.
        const sigma = 10;
        const rho = 28;
        const beta = 8 / 3;
        ...
        // Map attractor state to target-ish position
      ...
      if (id === "singularity") {
        // Strong pull to origin with spiral.
        const r = Math.sqrt(px * px + pz * pz) + 1e-3;
        vx += (-px / r) * 1.8 * dts;
        vz += (-pz / r) * 1.8 * dts;
        vx += (-pz / r) * 1.0 * dts;
  ...
  const uniforms = useMemo(() => {
    return {
      ...
      uOpacity: { value: reducedMotion ? 0.72 : 0.85 },
    };
  }, [particleColor, reducedMotion]);

  useEffect(() => {
    uniforms.uColor.value = particleColor;
  }, [uniforms, particleColor]);
    ...
    <points ref={meshRef}>
      <bufferGeometry>
        <bufferAttribute
          ...
          args={[stateRef.current.positions, 3]}
        />
...
export function WebGLParticleDeck() {
  ...
  const onPerfSample = useCallback((fps: number) => {
    // If we start dipping under ~42fps, degrade post FX (disable trails + lower bloom).
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-8 right-8 font-mono text-xs text-white/25 z-20">
        {current + 1} / {SLIDES.length}
      </div>

      <div className="absolute top-6 left-6 font-mono text-xs text-white/35 z-20">
        Keys: ← → Space | N HUD | R restart | Esc back
      </div>

      {showHud && (
        <div className="absolute top-14 left-6 z-30 w-[340px] rounded-xl border border-white/10 bg-black/40 backdrop-blur px-4 py-3 text-white/80">
