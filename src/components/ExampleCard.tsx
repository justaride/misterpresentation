...
export function ExampleCard({ example }: { example: Example }) {
  ...
  return (
    <Link to={`/examples/${example.id}`} className="block h-full">
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="h-full border-2 border-border bg-card shadow-[4px_4px_0px_0px_rgba(17,24,39,1)] flex flex-col hover:-translate-y-1 hover:shadow-[6px_6px_0px_0px_rgba(17,24,39,1)] hover:border-accent transition-all duration-200 cursor-pointer"
      >
        <div
        ...
        <div className="p-4 flex flex-col flex-1 gap-3">
          <div>
            ...
            <h3 className="font-display text-xl uppercase mt-1 group-hover:text-accent transition-colors">
              {example.title}
            </h3>
          </div>
